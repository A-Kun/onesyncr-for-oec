...
def push_file(path, commit_msg, content, token):
    try:
        ...
        if not repo:
            target_repo = gh.repository(TARGET_USERNAME,
                                        "open_exoplanet_catalogue")
            target_repo.create_fork()
            repo = gh.repository(user.login, "open_exoplanet_catalogue")
        github_api_base_url = "https://api.github.com/repos/" + user.login +\
                              "/open_exoplanet_catalogue/contents/"
        try:  # file does not exist, create the file
            repo.create_file(path, commit_msg, content.encode())
            print("create", end=" ", flush=True)
            return True
        # file already exists, update the file instead
        except github3.models.GitHubError:
            response = requests.get(github_api_base_url + path +
                                    QUERY_STRING).content
            sha = json.loads(response.decode())["sha"]
            remote_content = base64.b64decode(json.loads(response.decode())
